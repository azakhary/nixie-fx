...
// The alpha revert removed sampler forwarding; preserve direct texture reads.
assert.ok(
  !source.includes("sampler2D source"),
  "Do not restore sampler forwarding",
);
...
void main() {
  gl_FragColor = texture2D(secondTexture,
    vec2(texture2D(firstTexture, vec2(0.5)).r, 0.5));
}`,
      );
  ...
  assert.ok(
    result.pixels[1].pixel.every(
      (value, index) => Math.abs(value - [0, 0, 64, 128][index]) <= 1,
    ),
    "Direct texture reads must preserve stored RGBA",
  );
  assert.ok(
